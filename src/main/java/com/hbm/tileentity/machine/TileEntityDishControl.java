...
public class TileEntityDishControl extends TileEntityMachineBase implements IControlReceiver, IGUIProvider, SimpleComponent, CompatHandler.OCComponent {
	...
	{
		isLinked = establishLink(stack);
	}

	private boolean establishLink(ItemStack stack) {
		if(stack.getItem() != null && stack.stackTagCompound != null) {
			int xCoord = stack.stackTagCompound.getInteger("x");
    		int yCoord = stack.stackTagCompound.getInteger("y");
    		int zCoord = stack.stackTagCompound.getInteger("z");

    		Block b = worldObj.getBlock(xCoord, yCoord, zCoord);

    		if(b == ModBlocks.machine_stardar) {

    			int[] pos = ((MachineStardar) ModBlocks.machine_stardar).findCore(worldObj, xCoord, yCoord, zCoord);
    			...
    			if(pos != null) {
					...
					if(tile instanceof TileEntityMachineStardar) {
						...
						System.out.println("Dish linked at " + pos[0] + "," + pos[1] + "," + pos[2]);
						return true;
					}
				}
    		}
